...
def kfz():
    ...
    while 1:
        ...
        print("\033[93mD.管理磁盘  \033[91mK.Pan激活工具(谨慎使用) \033[94mP.系统密码破解 \033[92mM.磁盘引导修复")
        ...
        if kfz=="1":
            ...
            b("正在修复磁盘(C: Pan TSE 2.1)...")
            sleep(1)
            b("正在修复磁盘(D: Pan Me Beta)...")
            sleep(1)
            b("正在修复磁盘(E: Pan Me Beta)...")
            sleep(1)
            b("修复成功！重启将生效！")
        elif kfz=="2":
...
try:
    ...
    if panme=="3":
        ...
        name="Owner"
        while 1:
            print("\033[90mPan Preview version\nAssessment copy.Overdue time 2021/8/19.\033[0m")
            if classic==0 or classic==2:
                if classic==2:
                    print("\033[101m")
                    qing()
                print("\033[0mStart                        "+name)
                print("\033[90m0.Shutdown  \033[91m1.Calculator")
                print("\033[31m2.Notepad     \033[92m3.Restart")
                print("\033[32m4.System property    ")
                print("\033[34m5.Computer      \033[35m6.Pan introduction \033[96m7.||||")
                prog=input("\033[0mEnter program's number:")
            else:
                print("\033[103m")
                qing()
                print("It's ",ctime())
            ...
            if prog=="0":
                ...
                while 1:
                    qing()
                    print("\033[?25h\033[32mWelcome to notepad!\033[34mEnter notepad's name to open,\033[33mEnter new name to create,\033[31mEnter exit to exit,\033[35mEnter recycle to check the recycle bin")
                    ...
                    if j.lower()=='exit':
                        ...
                        print("\033[36m| Pan Recovered files\033[32m")
                        for i in jsbrc:
                            print("\033[32mNotepad's name : "+str(i))
                            print("\033[33mNotepad's content : See to downside\n"+str(jsbrc[i]))
                            print()
                        input("\033[36m(recovery contents cannot restore,You can read only,in the contents,\033[34m\\n\033[36mis for newline;Press Enter to return)")  
                    elif j.lower()=='recycle':
                        print("\033[36m| Recycle bin\033[32m")
                        for i in jsbr:
                            print(i)
                        cao = input("\033[36mEnter the operating(1=Restore 2=Delete 3=Delete to all 4=Restore to all):")
                        if cao != '3' and cao != '4':
                            jj = input("\033[33mEnter notepad's name:")
                        elif cao == '1':
                            if jj in jsbr:
                                jsb[jj] = jsbr[jj]
                                del jsbr[jj]
                                print("\033[32mSuccessfully to restore!")
                            else:
                                print("\033[31mError")
                        elif cao == '2':
                            if jj in jsbr:
                            ...
                            del jsbr
                            jsbr = {}
                            print("\033[36mRestoring......")
                            sleep(0.5)
                            print("\033[32mSuccessfully to restore!")
                        else:
                            print("\033[31minvalid operating")
                        input("\033[35mpress Enter to return")
                    elif j in jsb:
                        print(jsb[j])
                        ...
                        if c=='1':
                            ...
                            jsbr[j] = jsb[j]
                            del jsb[j]
                            print("\033[32mSuccessfully to delete,you can check it in recycle bin!")
                        elif c=='3':
                ...
                print("\033[0m💽 CD-ROM Driver(F:)")
                if ds_3==1:
                    print("0Byte free for 6.55GB UDF Disk format")
                else:
                    print("Driver cannot be use RAW Disk format")
                System32=input("Enter filename(sample:\"C:\"):")
                qing()
                if System32=="C:":
                    print("Name    Type            Modify date            Size")
                    ...
                    if System32=="F:":
                        print("Cannot read this disk, check that the disk is unplugged or moved, and check that disk management software is installed.")
                    else:
                        print("Bad command or file name.")
            elif prog=="6":
                ...
                qing()
                if rm=="2":
                    b("PanMe入门版是一款被大幅度精简了无用内容的特色装机系统，精简过后的PanMe入门版的体积被大幅度的消减但是运行的流畅度却没有因为精简而有丝毫的影响。")
                    b('''PanME入门版版特色：
1、开机速度相比Pan其他的操作系统提升了47%，更加安全可靠。
2、Pan NT 5.x内核的操作系统改进了基于角色的计算方案和用户账户管理。
3、用户体验将更加高级,还会整合自动化应用程序提交和交叉程序数据透明性。
4、PanMe大幅缩减了Pan的启动速度，系统加载时间一般不超过15秒。
5、PanMe将会让搜索和使用信息更加简单，包括本地、网络和互联网功能。
6、在数据保护和坚固协作的固有冲突之间搭建沟通桥梁,同时也会开启工作室级的数据保护和权限许可。''')
                    b('''PanMe入门版亮点：
1、开始菜单加入新选项和新功能。
2、记事本输入法上下文词语联想提升、触摸键盘提升。
3、引入边缘模糊UI，标准键盘已经支持英语、日语、简中、繁中四种常用语言。
4、设置改进和提升、声音设置挪到了启动管理器中、Pan更新改进
5、浏览器标签全自动静音
6、流线型文本引入，书写更流畅，词语预测更准确，在字符之间划画竖线的话，代表空格距离加大。(仅限Windows 7以上的系统使用)
7、浏览器、资源管理器等都加入了这一功能，只要设备支持蓝牙即可，然后就可以向临近的设备共享数据。
''')
                    b("PanMe入门版说明：\nPanMe入门版系统可以为用户带来更加流畅的使用并且各种系统的运行也绝不会因为精简而出现各种问题。")
                    b("1.以Pan TSE 2.0为母盘精简化\n2.不加载多余dll\n3.禁用第三方应用和应用商店\n4.关闭自动更新\n5.优化磁盘格式系统\n6.仅保留管理员，享受最高权限(无密码)\n7.日常使用基本无蓝屏\n8.系统经过优化，命令提示符启动更能提高查错(debug)效率")
                elif rm=="1":
                    b("1.Take Pan TSE 2.0 as the mother disk to streamline\n2.Don't load too much .dll file\n3.Disabled third party application and App store")
                ...
                while 1:
                    ...
                    if prog=="A":
                        ...
                        if llqjh==0:
                            b("\033[91mThemes setting had been disabled,because you haven't active Pan\033[0m")
                        prog=input()
                        qing()
                        print("\033[7m        Pan Boot Manager            ")
                        print("\033[0m",end="")
                        if prog=="A":
                            ...
                            if llqjh==0:
                                b("Enter your PAN key, which is in the PAN installation disc or in the developer's message")
                                b("sample:111-1111111")
                                key=input()
                                if len(key)!=11:
                                    key="Error"
                                else:
                                    ...
                                    while key=="Error":
                                        ...
                                        for i in range(0,len(key)):
                                            if key[i]!="-" and i==3:
                                                ...
                                                break
                                qing()
                                print("Successfully to sumbit key!")
                                print("Administrator (180*****260) being validated...")
                                if randint(0,10)==8:
                                    print("Administrator validation fails!Pan Team (189*****119) being validated......")
                                sleep(5)
                                llqjh=1
                                print("Key is enabled,successfully to active!")
                        ...
                        if llqjh==1:
                            if prog=="C":
                                ...
                                if prog=="A":
                                    ...
                                    speak("System Sound sample,the talker is Steve.Talker system test:Hello,Thank you,Would you please go.That is end of test.")
                            elif prog=="D":
                                b("Screen.currentResolution:874×560")
                                b("\033[90mChoise a Resolution(Disabled):\nA.1366×768\nB.1024×768\nC.874×560(推荐)\nD.800×600\nE.640×480")
                            elif prog=="E":
                                b(ctime())
                                ...
                                if classic==1:
                                    b("Theme:Pan classic")
                                b("更改主题:A.Pan Classic B.Theme property")
                                prog=input()
                                qing()
                ...
                try:
                    software2 = int(prog)
                    YUAN = "You entered an illegal number, please enter the correct number ~"
                except:
                    if prog == "" or prog == " " or prog == " "*software_len:
                        YUAN = "You didn't enter anything ~"
                    elif prog in NUMBER:
                        YUAN = "Please enter Arabic numerals, not Chinese ~"
                    elif prog[0:3] == "www" or prog[0:4] == "http":
                        YUAN = "URL input is not supported ~"
                    else:
                        YUAN = "You entered the program name, please enter the number ~"
                qing()
                print()
                print()
                print("\033[33mProgram not found - {} -".format(prog))
                print("\033[36mReason:{}".format(YUAN))
                sleep(0.5)
    ...
    if panme=="2":
        ...
        while 1:
            ...
            if prog=="0":
                ...
                print("\033[0mHOTM Pan[版本5.11.4012]")
